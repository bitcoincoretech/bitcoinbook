...
((("biblioteca pycoin")))Biblioteca Python http://github.com/richardkiss/pycoin[+pycoin+], scrisă inițial și întreținută de Richard Kiss, este o bibliotecă bazată pe Python care acceptă manipularea cheilor bitcoin și a tranzacțiilor, chiar și suportarea limbajului de script suficient pentru a face față corect tranzacțiilor non standard.

Biblioteca pycoin acceptă atât Python 2 (2.7.x) cât și Python 3 (3.3 și ulterioare) și vine cu câteva utilitare pentru linia de comandă, +ku+ și +tx+.

=== Utilitar pentru Chei (Key Utility - KU)

((("utilitare pentru chei (ku)", id="keyutil17")))Utilitarul din linia de comandă +ku+ ("key utility") este ca un cuțit elvețian pentru manipularea cheilor. Acceptă cheile BIP-32, WIF și adrese (monede bitcoin și altcoin). Urmează câteva exemple.

Crează o cheie BIP-32 folosind sursele de entropie implicite ale GPG și _/dev/random_:
...
[ATENȚIE]
====
Fraza de acces din acest exemplu este mult prea ușor de ghicit.
====
...
Verificați dacă funcționează prin alegerea unui șir BIP32 (cel corespunzător subcheiei 0/3):
...
((("", startref="pycoin library")))Ca adresă Dogecoin:
...
((("utilitar pentru tranzacții (TX)", id="TX17")))
Utilitarul de la linia de comandă +tx+ va afișa tranzacții sub o formă care poate fi citită de oameni, va prelua tranzacțiile de bază din cache-ul Pycoin sau de la servicii web (blockchain.info, blockcypher.com, blockr.io și chain.so sunt acceptate în prezent), va uni tranzacțiile , va adăuga sau șterge intrări sau ieșiri și va semna tranzacțiile tranzacțiile.
...
Nu se face automat, astfel încât un instrument de linie de comandă nu va scurge informații potențial private despre tranzacțiile care vă interesează pe un site terț. Dacă nu vă pasă, puteți introduce aceste rânduri în _.profile_.
...
Linia finală apare pentru că pentru a valida semnăturile tranzacțiilor, aveți tehnic nevoie de tranzacțiile sursă. Deci, să adăugăm +-a+ pentru a augmenta tranzacțiile cu informații sursă:
...
((("", startref="TX17")))Acum, să ne uităm la ieșirile necheltuite pentru o anumită adresă (UTXO). În blocul #1, vedem o tranzacție coinbase la +12c6DSiU4Rq3P4ZxziKxzrL5LmMBrzjrJX+. Să folosim +fetch_unspent+ pentru a găsi toate monedele de la această adresă:
